...
describe('Decisions 115–116 native policy derivation runner', () => {
  it('accepts only the exact closed command grammar', async () => {
    ...
    await expect(
      runPolicyNativeDerivationCommand(
        ['diagnose-a', '--confirm-m45-policy-native-a-diagnostic-v1'],
        seams,
      ),
    ).rejects.toThrow()
    await expect(
    ...
    await expect(
      runPolicyNativeDerivationCommand(
        ['diagnose-a', '--confirm-m45-policy-native-a-diagnostic-v6'],
        seams,
      ),
    ).rejects.toThrow()
    await expect(
    ...
    await expect(
      runPolicyNativeDerivationCommand(
        ['diagnose-a', '--confirm-m45-policy-native-a-diagnostic-v8'],
        seams,
      ),
    ).rejects.toThrow()
    await expect(
  ...
  it('admits only the exact v3 FD-map diagnostic grammar and closed union projection', async () => {
          ...
          ],
          {
            ...
            revalidateTracked: vi.fn(async (_root, expected) => {
              ...
              return trackedChecks === driftAt
                ? { ...expected, runnerSha256: 'f'.repeat(64) }
                : expected
            }),
          },
        ),
    ...
    for (const args of [
      ...
      [
        ...
        '--confirm-m45-policy-native-a-fd-map-scratch-recovery-v2',
      ],
      [
        'diagnose-a-fd-map',
  ...
  it('retires both consumed FD-map literals before every default or injected boundary', async () => {
    ...
    const grammar = runner.slice(
      runner.indexOf('function parseArguments'),
      runner.indexOf('function publicCommitments'),
    )
    ...
    const command = runner.slice(
      ...
      runner.indexOf('export async function runPolicyNativeDerivationCommand'),
    )
    expect(command.indexOf('parseArguments(argv)')).toBeLessThan(
      command.indexOf('filesystem.realpath'),
    )
    ...
    try {
      ...
      for (const boundary of [
        rejected.filesystem.realpath,
        rejected.filesystem.lstat,
        rejected.filesystem.readdir,
        heldDirectory,
        heldFile,
        heldMetadataFile,
