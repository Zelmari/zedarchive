...
export default function WeeklyCalendarModal({
  ...
  return (
    <div
      className="animate-fade-in fixed inset-0 z-[var(--za-layer-modal)] flex items-center justify-center bg-backdrop p-3 md:p-6"
      onClick={onClose}
    >
      <div
        ref={modalRef}
        className="flex max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-layered border border-required bg-surface shadow-layered"
        onClick={(e) => e.stopPropagation()}
        role="dialog"
        aria-modal="true"
        aria-labelledby="weekly-calendar-modal-title"
      >
        {/* Header */}
