...
- **Privacy Toggle:** Private by default. Toggle between _Public_ and _Private_ at any time in the Share modal.
...
```bash
cp .env.example .env.local   # then set DATABASE_URL to the docker URL shown inside
npm run setup                # postgres up → migrate → seed demo data → next dev
```

- Demo login: `demo@zedarchive.com` / `password123` (public handle `@zelmari`)
- Stop the stack with `npm run docker:down`
- Re-seed anytime with `npm run db:seed` (idempotent; refuses non-local hosts unless `--force`)
